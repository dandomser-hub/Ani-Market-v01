...
import { acceptExcessAdjustment, getFulfillmentRecords, getGate1Transactions, recordFulfillment } from '../../data/gate1CommerceData';
import { getCureRecord, releaseOutstandingAfterCure, requestSupplierCure, resolveCureIfFulfilled } from '../../data/gate1FlowData';
import { getPlatformFeeMaturitySummary, getPlatformFeeObligation } from '../../data/gate2bPlatformFeeData';
...
export default function Gate1TransactionWorkspace() {
  ...
  const feeObligation = getPlatformFeeObligation(transaction.id);
  const feeSummary = getPlatformFeeMaturitySummary(transaction.id);
  const today = new Date().toISOString().slice(0, 10);
  const cureEligible = transaction.activeCommittedQuantity > 0 && (transaction.presentedQuantity > 0 || today >= terms.fulfillmentDate);
  ...
  const recordAcceptance = () => {
    ...
    if ('error' in result) setMessage(result.error ?? 'Unable to record fulfillment.');
    else {
  ...
  const requestCure = () => {
    ...
    setMessage('error' in result ? (result.error ?? 'Unable to request Supplier cure.') : 'Supplier cure opportunity recorded. Outstanding quantity remains Active Committed until cured or formally released.');
  ...
  const releaseAfterCure = () => {
    ...
    setMessage('error' in result ? (result.error ?? 'Unable to release outstanding quantity.') : `${result.releasedQuantity.toLocaleString()} ${terms.unit} released back to Demand Remaining Quantity.`);
    setRevision(value => value + 1);
  };
  ...
  const acceptExcess = () => {
    ...
    setMessage('error' in result ? (result.error ?? 'Unable to record accepted excess.') : 'Accepted Excess Adjustment recorded. It is now included in Fulfilled quantity and Final Transaction Value.');
    setRevision(value => value + 1);
  };
    ...
    <div className="mx-auto max-w-5xl space-y-5">
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
        <div className="space-y-5 lg:col-span-2">
          <div className="card"><h2 className="section-title mb-4">Immutable Final Terms Snapshot</h2><div className="grid grid-cols-2 gap-4 sm:grid-cols-4"><div><div className="text-xs text-gray-500">Buyer</div><div className="font-semibold">{terms.buyerName}</div></div><div><div className="text-xs text-gray-500">Supplier</div><div className="font-semibold">{terms.supplierName}</div></div><div><div className="text-xs text-gray-500">Commodity</div><div className="font-semibold">{terms.cropName}</div></div><div><div className="text-xs text-gray-500">Committed</div><div className="font-semibold">{terms.committedQuantity.toLocaleString()} {terms.unit}</div></div><div><div className="text-xs text-gray-500">Agreed Price</div><div className="font-semibold text-green-700">₱{terms.agreedTransactionPrice.toLocaleString()}</div></div><div><div className="text-xs text-gray-500">Committed Value</div><div className="font-semibold">₱{transaction.committedTransactionValue.toLocaleString()}</div></div><div><div className="text-xs text-gray-500">Fulfillment</div><div className="font-semibold">{terms.fulfillmentDate}</div></div><div><div className="text-xs text-gray-500">Proposal Version</div><div className="font-semibold">v{terms.negotiationProposalVersion}</div></div></div>{terms.specificationVariations && <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">Agreed variation: {terms.specificationVariations}</div>}</div>
          <div className="card"><h2 className="section-title mb-4">Fulfillment Quantity Record</h2><div className="grid grid-cols-2 gap-3 sm:grid-cols-4"><div><span className="text-xs text-gray-500 block">Historical Committed</span><strong>{transaction.historicalCommittedQuantity.toLocaleString()} {terms.unit}</strong></div><div><span className="text-xs text-gray-500 block">Active Outstanding</span><strong className="text-teal-700">{transaction.activeCommittedQuantity.toLocaleString()} {terms.unit}</strong></div><div><span className="text-xs text-gray-500 block">Presented</span><strong>{transaction.presentedQuantity.toLocaleString()} {terms.unit}</strong></div><div><span className="text-xs text-gray-500 block">Buyer Accepted</span><strong className="text-green-700">{transaction.acceptedQuantity.toLocaleString()} {terms.unit}</strong></div><div><span className="text-xs text-gray-500 block">Rejected / Unaccepted</span><strong>{transaction.rejectedQuantity.toLocaleString()} {terms.unit}</strong></div><div><span className="text-xs text-gray-500 block">Accepted Excess</span><strong>{transaction.acceptedExcessQuantity.toLocaleString()} {terms.unit}</strong></div><div><span className="text-xs text-gray-500 block">Released Shortfall</span><strong>{transaction.releasedShortfallQuantity.toLocaleString()} {terms.unit}</strong></div><div><span className="text-xs text-gray-500 block">Final Transaction Value</span><strong>₱{transaction.finalTransactionValue.toLocaleString()}</strong></div></div></div>
          {isBuyer && <div className="card border-green-200"><h2 className="section-title mb-4">Record Presented / Accepted Quantity</h2><div className="grid grid-cols-1 gap-3 sm:grid-cols-3"><div><label className="label">Presented</label><input type="number" className="input" value={presented} onChange={event => setPresented(event.target.value)} /></div><div><label className="label">Accepted</label><input type="number" className="input" value={accepted} onChange={event => setAccepted(event.target.value)} /></div><div><label className="label">Rejected / Unaccepted</label><input type="number" className="input" value={rejected} onChange={event => setRejected(event.target.value)} /></div></div><div className="mt-3"><label className="label">Remarks</label><textarea className="input resize-none" rows={2} value={remarks} onChange={event => setRemarks(event.target.value)} /></div><button onClick={recordAcceptance} className="btn-primary mt-3"><CheckCircle size={15} /> Save Buyer Acceptance</button><p className="mt-3 text-xs text-gray-500">Normal acceptance cannot exceed Active Committed quantity. Any presented excess is non-payable/non-fulfilling until explicitly accepted against legitimate outstanding Demand.</p></div>}
        ...
        <div className="space-y-5">
          <div className="card"><h3 className="font-semibold text-gray-800">Commercial Values</h3><div className="mt-3 space-y-2 text-sm"><div className="flex justify-between"><span>Committed Transaction Value</span><strong>₱{transaction.committedTransactionValue.toLocaleString()}</strong></div><div className="flex justify-between"><span>Final Transaction Value</span><strong>₱{transaction.finalTransactionValue.toLocaleString()}</strong></div></div><p className="mt-3 text-xs text-gray-500">Final value is based on Buyer-confirmed Accepted Quantity plus valid Accepted Excess Adjustments. Buyer-to-Supplier payment remains separate from Supplier-to-Ani Market platform-fee settlement.</p></div>
          {feeObligation && <div className="card border-teal-200"><h3 className="font-semibold text-gray-800">Success-Based Platform Fee — Read Only</h3><div className="mt-3 space-y-2 text-sm"><div className="flex justify-between"><span>Locked Rate</span><strong>{feeObligation.lockedRatePercent.toFixed(2)}%</strong></div><div className="flex justify-between"><span>Final Fee Obligation</span><strong>{feeObligation.originalFeeAmount === undefined ? 'Pending FTV finalization' : `₱${feeObligation.originalFeeAmount.toLocaleString()}`}</strong></div><div className="flex justify-between"><span>Matured Fee</span><strong>₱{(feeSummary?.maturedAmount ?? 0).toLocaleString()}</strong></div><div className="flex justify-between"><span>Not Yet Due</span><strong>₱{(feeSummary?.notYetDueAmount ?? feeObligation.originalFeeAmount ?? 0).toLocaleString()}</strong></div><div className="flex justify-between"><span>Current Due</span><strong>₱{(feeSummary?.currentDueAmount ?? 0).toLocaleString()}</strong></div><div className="flex justify-between"><span>Overdue</span><strong>₱{(feeSummary?.overdueAmount ?? 0).toLocaleString()}</strong></div><div className="flex justify-between"><span>Fee State</span><strong>{feeSummary?.status ?? feeObligation.status}</strong></div>{feeSummary?.nextDueAt && <div className="flex justify-between"><span>Next Due</span><strong>{new Date(feeSummary.nextDueAt).toLocaleString()}</strong></div>}</div><p className="mt-3 text-xs text-gray-500">Fee maturity is triggered only by Supplier-confirmed Buyer receipts after Final Transaction Value is authoritative. 2B-2 does not collect the fee or enforce delinquency.</p></div>}
          ...
          {isBuyer && cureEligible && <div className="card border-amber-200"><h3 className="font-semibold text-amber-900 flex items-center gap-2"><AlertTriangle size={16} /> Under-Fulfillment / Cure</h3><p className="mt-2 text-xs text-amber-800">A fulfillment shortfall is now evident because quantity has been presented short or the agreed fulfillment date has arrived/passed. Outstanding quantity remains Active Committed during the Supplier cure opportunity.</p><div className="mt-3 space-y-2">{!cure || cure.status !== 'Requested' ? <button onClick={requestCure} className="btn-secondary w-full justify-center text-xs">Request Supplier Cure</button> : <><div className="rounded-lg bg-amber-50 p-2 text-xs text-amber-800">Cure requested: {cure.reason}</div><button onClick={releaseAfterCure} className="btn-danger w-full justify-center text-xs">Cure Failed — Release Outstanding</button></>}</div></div>}
