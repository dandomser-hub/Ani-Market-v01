...
export default function ForBuyers() {
  const benefits = [
    { icon: <FileText size={24} className="text-brand-primary" />, title: 'Post Your Exact Requirement', desc: 'Define crop, variety, quantity, target price, quality specs, and timeline. Suppliers respond to YOUR need.' },
    { icon: <Search size={24} className="text-brand-primary" />, title: 'Verified Supplier Pool', desc: 'Browse responses from verified farmers, cooperatives, and organized suppliers in Mainland Bicol.' },
    { icon: <GitMerge size={24} className="text-brand-primary" />, title: 'Clean One-to-One Matching', desc: 'MVP uses one demand matched to one supplier — no partial fulfillment confusion in this phase.' },
    { icon: <UploadCloud size={24} className="text-brand-primary" />, title: 'Transparent Payment Recording', desc: 'Payment reference numbers and proof uploaded for evidence. No hidden fund processing.' },
    { icon: <ShieldCheck size={24} className="text-brand-primary" />, title: 'Admin Support', desc: 'Platform admin available for disputes, cancellations, and exceptional cases.' },
  ];
  ...
  return (
    <div className="bg-white">
      <div className="bg-white text-brand-ink py-16 border-b border-green-100">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {benefits.map(b => (
            <div key={b.title} className="bg-white rounded-xl p-6 border border-green-100 shadow-sm">
