...
const PROTOTYPE_USER_STORAGE_KEY = 'ani-market-gate1-prototype-users';
...
export const gate1TrustStates: Record<string, Gate1TrustState> = {
  u1: {
    ...
    buyerProfile: { userId: 'u1', organizationName: 'Naga Valley Rice Mill', businessType: 'Rice Mill', authorizedRepresentative: 'Procurement Officer', procurementAddress: 'Naga City, Camarines Sur' },
  ...
  u2: {
    ...
    supplierProfile: { userId: 'u2', supplierType: 'cooperative', farmOrOrganizationName: 'Polangui Farmers Association', operatingLocation: 'Polangui, Albay', cropInterests: ['Rice', 'Abaca', 'Other Crops'] },
  ...
  u3: {
    accountVerification: verifiedAccount,
    roleVerifications: { supplier: enabledRole('supplier') },
    supplierProfile: { userId: 'u3', supplierType: 'individual_farmer', farmOrOrganizationName: 'Pedro Santos Farm', operatingLocation: 'Nabua, Camarines Sur', cropInterests: ['Rice'] },
    roleContext: { activeRole: 'supplier', availableRoles: ['supplier'] },
    onboardingProgress: { supplier: 'Complete' },
  },
  u4: {
    accountVerification: verifiedAccount,
    roleVerifications: { buyer: enabledRole('buyer') },
    buyerProfile: { userId: 'u4', organizationName: 'Bicol Agri Trading Corp.', businessType: 'Agri Enterprise', authorizedRepresentative: 'Procurement Manager', procurementAddress: 'Legazpi City, Albay' },
    roleContext: { activeRole: 'buyer', availableRoles: ['buyer'] },
    onboardingProgress: { buyer: 'Complete' },
  },
  u5: {
    ...
    supplierProfile: { userId: 'u5', supplierType: 'cooperative', farmOrOrganizationName: 'Daet Coconut Cooperative', operatingLocation: 'Daet, Camarines Norte', cropInterests: ['Coconut'] },
    roleContext: { activeRole: 'supplier', availableRoles: ['supplier'] },
  ...
  u6: {
    ...
    roleVerifications: {
      ...
      buyer: { role: 'buyer', profileCompleteness: 'In Progress', marketplaceVerificationStatus: 'Not Submitted', transactionAccessStatus: 'Disabled' },
    },
    supplierProfile: { userId: 'u6', supplierType: 'individual_farmer', farmOrOrganizationName: 'Maria Cruz Farm', operatingLocation: 'Libmanan, Camarines Sur', cropInterests: ['Rice', 'Vegetables'] },
  ...
  u7: {
    accountVerification: verifiedAccount,
    roleVerifications: { supplier: enabledRole('supplier') },
    supplierProfile: { userId: 'u7', supplierType: 'aggregator', farmOrOrganizationName: 'Sorsogon Agri Aggregators', operatingLocation: 'Sorsogon City, Sorsogon', cropInterests: ['Coconut', 'Spices'] },
    roleContext: { activeRole: 'supplier', availableRoles: ['supplier'] },
    onboardingProgress: { supplier: 'Complete' },
  },
  u8: {
    ...
    buyerProfile: { userId: 'u8', organizationName: 'Irosin Spice Traders', businessType: 'Trading Business', procurementAddress: 'Irosin, Sorsogon' },
    roleContext: { activeRole: 'buyer', availableRoles: ['buyer'] },
...
export const gate1VerificationRecords: VerificationRecord[] = [
  { id: 'vr-u5-supplier-1', userId: 'u5', role: 'supplier', toStatus: 'Pending Review', eventType: 'Submitted', actorId: 'u5', actorRole: 'supplier', createdAt: '2026-08-22T14:00:00+08:00' },
  { id: 'vr-u8-buyer-1', userId: 'u8', role: 'buyer', toStatus: 'Pending Review', eventType: 'Submitted', actorId: 'u8', actorRole: 'buyer', createdAt: '2026-08-22T14:10:00+08:00' },
];

function readTrustOverrides(): Record<string, Partial<Gate1TrustState>> {
  if (typeof window === 'undefined') return {};
  try { return JSON.parse(window.localStorage.getItem(TRUST_STORAGE_KEY) ?? '{}') as Record<string, Partial<Gate1TrustState>>; }
  catch { return {}; }
}
...
function userTrustState(user: User): Gate1TrustState {
  return {
    accountVerification: user.accountVerification,
    roleVerifications: user.roleVerifications,
...
export function getPrototypeUsers(): User[] {
  if (typeof window === 'undefined') return [];
  try { return JSON.parse(window.localStorage.getItem(PROTOTYPE_USER_STORAGE_KEY) ?? '[]') as User[]; }
  catch { return []; }
}

export function savePrototypeUser(user: User) {
  if (typeof window === 'undefined') return;
  ...
  window.localStorage.setItem(PROTOTYPE_USER_STORAGE_KEY, JSON.stringify(users));
}

export function getVerificationRecords(userId?: string): VerificationRecord[] {
  if (typeof window === 'undefined') return userId ? gate1VerificationRecords.filter(record => record.userId === userId) : gate1VerificationRecords;
  let persisted: VerificationRecord[] = [];
  try { persisted = JSON.parse(window.localStorage.getItem(VERIFICATION_RECORD_STORAGE_KEY) ?? '[]') as VerificationRecord[]; }
  catch { persisted = []; }
...
export function enrichUserWithGate1Trust(user: User): User {
  const embedded = userTrustState(user);
  const baseline = mergeTrustState(embedded, gate1TrustStates[user.id] ?? {});
  const override = readTrustOverrides()[user.id] ?? {};
  const state = mergeTrustState(baseline, override);
  return { ...user, ...state };
}
